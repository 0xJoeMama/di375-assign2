...
\begin{center}
	\begin{circuitikz}
		...
		\tikzset{mux 4to1/.style={muxdemux, muxdemux def={NL=4, NT=0, NB=2, NR=1}, muxdemux label={L1=$00$, L2=$01$, L3=$10$, L4=$11$}}}
		\draw
		(7, 0) node[label] (f) {$f$}
		(f) ++ (-2, 0) node[mux 4to1] (mux) {}
		(mux) ++ (-3, 3) node[vcc] (vcc) {$V_{DD}$}
		(mux) ++ (-2, -4) node[ground] (gnd) {$GND$}

		(mux.lpin 2) ++ (-4, 0) node[label] (X) {$x$}
		(X) ++ (0, -3) node[label] (Y) {$y$}
